...
from geometry_msgs.msg import Twist  # Twist 메시지 타입 (cmd_vel용)
...
class CmdAndPose(Node):
    ...
    def __init__(self):
        ...
        self.sub_cmd_vel = self.create_subscription(Twist, '/turtle1/cmd_vel', self.callback_cmd_vel, 10)

        self.timer_period = 1.0
        self.timer = self.create_timer(self.timer_period, self.timer_callback)

        self.publisher = self.create_publisher(CmdAndPoseVel, 'turtle_cmd_and_pose', 10)
    ...
    def callback_cmd_vel(self, msg):
        """cmd_vel 토픽의 콜백 함수: linear.x와 angular.z를 커스텀 메시지에 저장"""
    ...
    def timer_callback(self):
        ...
        self.get_logger().info(f'Publishing: {self.cmd_pose}')
